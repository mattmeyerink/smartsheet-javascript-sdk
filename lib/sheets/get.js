...
exports.create = function(options) {
  ...
  var listSheets = (getOptions, callback) => {
    // The /sheets endpoint no longer supports a trailing slash
    // We need to slice off the trailing slash from the url if 
    // this method is invoked without an id or a preset url
    var urlOptions = {};
    if (!getOptions?.url && !getOptions?.id) {
      urlOptions = {
        url: options.apiUrls.sheets.substring(0, options.apiUrls.sheets.length)
      }
    }
